...
# def multi_task_dataloader(pivot, embeddings, comment_id, comment_mapping_dict):

#     def create_aligned_data(comment_ids):
#         embeddings_list = []
#         targets_list = []
#         masks_list = []
...
#             embed_idx = comment_mapping_dict[comment_id]
#             embeddings_list.append(embeddings[embed_idx])
   
#             target_row = pivot.loc[comment_id]
#             targets_list.append(target_row.fillna(0).values)
#             masks_list.append(target_row.notna().astype(int).values)

#         return (torch.tensor(np.array(embeddings_list), dtype=torch.float32),
#                 torch.tensor(np.array(targets_list), dtype=torch.float32),
#                 torch.tensor(np.array(masks_list), dtype=torch.float32))
    
#     emb, tar, mask = create_aligned_data(comment_id)    
#     dataset = MultiTaskDataset(emb, tar, mask)    
...
#     '''
#     Try to put the training data with the same comment_ids into the same batch
